...
import { FormLanguage, ServerCorpus, ServerLanguage, ServerSource } from "../../Model/DexterModel"
import { useDebounce } from "../../Utils/useDebounce"
import { deleteLanguageFromCorpus, deleteLanguageFromSource, getLanguagesAutocomplete } from "../API"
...
export const LanguagesField = (props: LanguagesFieldProps) => {
    ...
    const deleteLanguageHandler = (language: ServerLanguage) => {
        if (!props.edit) {
            if (props.setValueCorpus) {
                props.setValueCorpus("languages", selectedItems.filter((entry: ServerLanguage) => entry !== language))
            }

            if (props.setValueSource) {
                props.setValueSource("languages", selectedItems.filter((entry: ServerLanguage) => entry !== language))
            }
        }
    ...
    const deleteLanguageFromCorpusHandler = async (corpusId: string, language: ServerLanguage) => {
    ...
    const deleteLanguageFromSourceHandler = async (sourceId: string, language: ServerLanguage) => {
        const warning = window.confirm("Are you sure you wish to delete this language?")
                        ...
                        renderTags={(tagValue, getTagProps) =>
                            tagValue.map((language, index) => (
                                <Chip
                                    ...
                                    key={index}
                                    {...getTagProps({ index })}
                                    onDelete={() => { deleteLanguageHandler(language) }}
                                />
                            ))
                        }
                        onChange={(_, data) => {
