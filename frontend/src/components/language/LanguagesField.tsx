...
export const LanguagesField = (props: LanguagesFieldProps) => {
  const [options, setOptions] = React.useState<ResultLanguage[]>([]);
  ...
  function renderInputField(params: TextFieldProps): JSX.Element {
    return <TextField {...params} value={inputValue} size={'medium'} />;
  }
  ...
  return (
    <Autocomplete
      ...
      options={options}
      getOptionLabel={(language: ResultLanguage) => language.refName}
      filterOptions={x => x}
      ...
      onChange={(_, data) => {
        props.onChangeSelected(data as ResultLanguage[]);
      }}
